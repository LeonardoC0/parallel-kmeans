...
public:
    ...
    // Método para calcular o WCSS (inalterado)
    double calculateWCSS() const
    {
        double total_wcss = 0.0;
        #pragma omp_set_num_threads(NUM_THREADS);
        #pragma omp parallel for reduction(+ : total_wcss) schedule(dynamic)
    ...
    void run(vector<Point> &all_points)
    {
        ...
        // NOTA: srand(time(0)) pode ser um problema em loops paralelos
        // se as threads forem criadas no mesmo segundo.
        // Como o K-Means é instanciado DENTRO do loop paralelo, 
        // é melhor usar uma semente baseada no K (que é único por thread).
        srand(time(0) + K); // Semente única por K
        ...
        // Gravar o resultado do K=10 (ou qualquer K específico)
        // Isso é seguro em paralelo, pois cada thread escreve um arquivo
        // com nome diferente (baseado em K).
        if (K == 10) {
            ofstream pointsFile;
...
int main(int argc, char **argv)
{
    ...
    // --- Implementação do Método do Cotovelo (PARALELIZADO) ---
    const int K_MIN = 2; 
    const int K_MAX = 20; 
    const int IT_MAX = 200; 

    // É crucial ter uma cópia original para resetar o estado dos pontos
    const vector<Point> original_points = all_points; 
    vector<pair<int, double>> wcss_results; 

    // Calcula o K_MAX real, pois não podemos ter K > N
    int K_MAX_REAL = min(K_MAX, (int)original_points.size() - 1);

    if (K_MAX_REAL < K_MIN) {
        cout << "Error: Not enough data points (" << original_points.size() << ") to run K-Means (K_MIN=" << K_MIN << ")." << endl;
        return 1;
    }

    if ((int)all_points.size() <= K_MAX)
    {
        cout << "Warning: Number of points is less than K_MAX. Reducing K_MAX to " << K_MAX_REAL << endl;
    }
    
    cout << "\n--- Running Elbow Method (K=" << K_MIN << " to K=" << K_MAX_REAL << ") in Parallel ---\n" << endl;
    cout << "K | WCSS (Within-Cluster Sum of Squares)\n";
    cout << "------------------------------------------\n";

    // *** INÍCIO DA REGIÃO PARALELA ***
    // Paraleliza o loop 'K'. Cada thread executará um 'K' diferente.
    // 'schedule(dynamic)' ajuda a balancear a carga se K=2 for muito
    // mais rápido que K=20.
    #pragma omp_set_num_threads(NUM_THREADS);
    #pragma omp parallel for schedule(dynamic)
    for (int K = K_MIN; K <= K_MAX_REAL; ++K)
    {
        // 1. Resetar pontos: Cada thread usa a cópia original (const)
        // para criar sua própria cópia privada (current_points).
        vector<Point> current_points = original_points;
        
        // 2. Executar K-Means: Cada thread cria sua própria instância
        ...
        // 4. Salvar resultado (REGIÃO CRÍTICA)
        // Acesso ao 'wcss_results' (compartilhado) deve ser protegido
        // para evitar que duas threads escrevam ao mesmo tempo.
        #pragma omp critical
        {
            wcss_results.push_back({K, wcss});
        }
        
        // NÃO imprimimos 'cout' aqui, pois a saída ficaria bagunçada.
    }
    ...
    // Ordenar os resultados por K, pois a execução paralela
    // os insere no vetor fora de ordem.
    sort(wcss_results.begin(), wcss_results.end());

    // Imprimir os resultados ordenados
    for(const auto& result : wcss_results)
    {
         cout << fixed << setprecision(4) << result.first << " | " << result.second << endl;
    }
